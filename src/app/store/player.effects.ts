...
export class PlayersEffects {
  ...
  loadIndexDbPlayers$ = createEffect(() =>
    this.actions$.pipe(
      ofType(PlayerAction.loadIndexDbPlayers),
      exhaustMap(() =>
        from(this.indexDbService.readPlayers()).pipe(
          mergeMap((players) => [
  ...
  setIndexDbPlayers$ = createEffect(() =>
    this.actions$.pipe(
      tap((e) => console.log(e)),
      ofType(PlayerAction.storePlayer),
      concatMap((p) =>
        from(this.indexDbService.savePlayer(p)).pipe(
          map(() => PlayerAction.httpSyncPlayers()),
          catchError((e) =>
            of({ type: '[indexDBService] Save Player Error', e }),
          ),
        ),
      ),
    ),
  ...
  replacePlayer$ = createEffect(() =>
    this.actions$.pipe(
      ...
      concatMap((p) =>
        from(this.indexDbService.replacePlayer(p)).pipe(
          map(() => ({ type: '[IndexDb] changed Key' })),
          catchError((e) =>
            of({ type: '[http Service] changed Key error', e }),
          ),
        ),
      ),
    ),
    ...
    this.actions$.pipe(
      ...
      exhaustMap(() => {
        from(this.indexDbService.reset()).pipe(
          ...
          catchError((reason) =>
            of({ type: '[Players Effect] IndexDb Reset Failed', reason }),
          ),
        );
        return of(resetLocalGames()).pipe(
          catchError((reason) =>
            of({ type: '[Players Effect] Game Reset Failed', reason }),
          ),
        );
      }),
    ),
  ...
  validatePlayer$ = createEffect(() =>
    this.actions$.pipe(
      ofType(PlayerAction.validatePlayer),
      withLatestFrom(this.store.pipe(select('players'))),
      mergeMap((newAndCurr) =>
        this.playerIsValid(newAndCurr[0], newAndCurr[1]).pipe(
          mergeMap(() => [
            PlayerAction.storePlayer(newAndCurr[0]),
            regenTable(),
          ]),
          catchError((reason) =>
            of({ type: '[Players Effect] Player Not Valid', reason }),
          ),
        ),
      ),
    ),
  ...
  httpSyncPlayers$ = createEffect(() =>
    this.actions$.pipe(
      ofType(PlayerAction.httpSyncPlayers),
      withLatestFrom(this.store.pipe(select('players'))),
      concatMap((ps) =>
        this.mergeMapSyncPlayers(ps[1]).pipe(
          mergeMap((action) => [action, regenTable()]),
          catchError((error) => {
            console.log(error);
  ...
  pullPlayerHttp$ = createEffect(() =>
    this.actions$.pipe(
      ofType(PlayerAction.pullPlayerHttp),
      concatMap((id) =>
        this.httpService.getPlayer(id.payload).pipe(
          map((p) => PlayerAction.validatePlayer(p)),
          catchError((reason) =>
            of({ type: '[Players Effect] Http Pull Failed', reason }),
          ),
        ),
      ),
    ),
  ...
  newPulledGame = createEffect(() =>
    this.actions$.pipe(
      ofType(PlayerAction.newHttpPulledGame),
      withLatestFrom(this.store.pipe(select('players'))),
      concatMap((idAndState) =>
        this.playerExists(idAndState[0].payload, idAndState[1]).pipe(
          catchError((reason) =>
            of({ type: '[Players Effect] Http Pull Failed', reason }),
          ),
        ),
  ...
  mergeMapSyncPlayers(ps: Player[]): Observable<Player & Action<any>> {
    return from(ps.filter((x) => x.synced === false)).pipe(
      mergeMap((p) =>
        this.httpService.putItem<Player>(p).pipe(
          map((event) => {
  ...
  playerIsValid(p: Player, state: Player[]): Observable<boolean> {
    p.name.replace(/\s+/g, '');
    if (p.name.length > 36) return throwError(() => 'to long');
    if (p.name.length <= 3) return throwError(() => 'to short');
